...
left outer JOIN
                (SELECT  user_id,count(*) as totinserts
                 FROM  activitylog where type <> null and type = 'INSERT'
                 GROUP BY user_id) S1 
ON S1.user_id = user.id 
left outer JOIN
                (SELECT  user_id,count(*) as totupdates
                 FROM  activitylog where type <> null and type = 'UPDATE'
                 GROUP BY user_id) S2 
ON S2.user_id = user.id
left outer JOIN
                (SELECT  user_id,count(*) as totdeletes
                 FROM  activitylog where type <> null and type = 'DELETE'
                 GROUP BY user_id) S3 
...
left outer JOIN (select user_id,count(*) as upload_count from user_upload_dump where user_id <> null  group by user_id) S6
